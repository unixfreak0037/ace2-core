...
class ExceptionFormatter(object):
    ...
    AST_ELEMENTS = {
        ...
        "keywords": [
            getattr(ast, cls) for cls in dir(ast) if keyword.iskeyword(cls.lower()) and isast(getattr(ast, cls))
        ],
    }

    def __init__(self, colored=False, theme=THEME, max_length=MAX_LENGTH, pipe_char=PIPE_CHAR, cap_char=CAP_CHAR):
