...
class ACEService(object):
    def __init__(self, service_config=None, *args, **kwargs):
        ...
        self.service_indicator_path = os.path.join(ace.SERVICES_DIR, self.service_name)
    ...
    def execute_service_wrapper(self):
        ...
        if self.service_is_daemon:
            ...
            service_logging_config_path = os.path.join(ace.SAQ_HOME, 'etc', 'logging_configs', f'service_{self.service_name}.ini')
            default_service_logging_config_path = os.path.join(ace.SAQ_HOME, 'etc', 'logging_configs', f'service_{self.service_name}.default.ini')
...
def get_all_service_names():
    ...
    return sorted(list(set([_[len('service_'):] for _ in ace.CONFIG.sections() if _.startswith('service_')])))

def get_service_config(service_name):
    """Returns the config section (from ace.CONFIG) for the given service name,
       or None if the service is unknown."""
    try:
        return ace.CONFIG[f'service_{service_name}']
    except KeyError:
        return None
...
def get_service_pid_path(service_name):
    ...
    return os.path.join(ace.SERVICES_DIR, service_name)
...
def get_service_status(service_name):
    ...
    if service_pid is None:
        if not ace.CONFIG[f'service_{service_name}'].getboolean('enabled'):
