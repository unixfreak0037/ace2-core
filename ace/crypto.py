...
import ace
from ace.service import ACEService
...
def read_ecs():
    ...
    try:
        ...
        client_socket.connect(ace.ECS_SOCKET_PATH)
...
def encryption_key_set():
    ...
    from ace.configuration import get_database_config_value
    for key in [ 
...
def set_encryption_password(password, old_password=None, key=None):
    ...
    from ace.configuration import set_database_config_value
    ...
    if encryption_key_set():
        ...
        if old_password is not None:
            ...
            ace.ENCRYPTION_PASSWORD = get_aes_key(old_password)

    if ace.ENCRYPTION_PASSWORD is None:
        # otherwise we just make a new one
        if key is None:
            ace.ENCRYPTION_PASSWORD = Crypto.Random.OSRNG.posix.new().read(32)
        else:
            ace.ENCRYPTION_PASSWORD = key

    # now we compute the key to use to encrypt the encryption key using the user-supplied password
    salt = Crypto.Random.OSRNG.posix.new().read(ace.CONFIG['encryption'].getint('salt_size', fallback=32))
    iterations =  ace.CONFIG['encryption'].getint('iterations', fallback=8192)
    result = PBKDF2(password, salt, 64, iterations)
    ...
    encrypted_encryption_key = encrypt_chunk(ace.ENCRYPTION_PASSWORD, password=user_encryption_key)
...
def encrypt(source_path, target_path, password=None):
    """Encrypts the given file at source_path with the given password and saves the results in target_path.
       If password is None then ace.ENCRYPTION_PASSWORD is used instead.
       password must be a byte string 32 bytes in length."""

    if password is None:
        password = ace.ENCRYPTION_PASSWORD
...
class EncryptionCacheService(ACEService):
    def __init__(self, *args, **kwargs):
        if ace.ENCRYPTION_PASSWORD_PLAINTEXT is None:
            raise RuntimeError("missing password -- make sure you use the -p option")

        super().__init__(service_config=ace.CONFIG['service_ecs'], 
                         *args, **kwargs)

    def initialize_service_environment(self):
        if os.path.exists(ace.ECS_SOCKET_PATH):
            os.remove(ace.ECS_SOCKET_PATH)

    def execute_service(self):
        logging.info(f"starting encryption cache service on {ace.ECS_SOCKET_PATH}")
        while not self.is_service_shutdown:
            try:
                ...
                server.settimeout(1)
                server.bind(ace.ECS_SOCKET_PATH)
                server.listen(5)

                os.chmod(ace.ECS_SOCKET_PATH, 0o600)
                ...
                while not self.is_service_shutdown:
                    ...
                    try:
                        ...
                        client_socket.send(f'{ace.ENCRYPTION_PASSWORD_PLAINTEXT}\n'.encode('utf8'))
    ...
    def stop_service(self, *args, **kwargs):
        ...
        try:
            ...
            with open(ace.ECS_SOCKET_PATH, 'r') as fp:
                pass
        except:
            pass
