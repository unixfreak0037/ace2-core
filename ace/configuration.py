...
class EncryptedPasswordInterpolation(Interpolation):
    def before_get(self, parser, section, option, value, defaults):
        ...
        if not hasattr(ace, 'ENCRYPTION_INITIALIZED'):
            return value

        if not ace.ENCRYPTION_INITIALIZED:
            return value
...
class ACEConfigParser(ConfigParser):
    ...
    def load_file(self, path):
        ...
        Args:
            path: The path to the ini formatted file to load. 
...
def load_configuration():
    ...
    #if ace.UNIT_TESTING:
        #unittest_passwords_path = os.path.join(ace.SAQ_HOME, 'etc', 'unittest.passwords.json')
    ...
    default_config.load_file(os.path.join(ace.SAQ_HOME, 'etc', 'saq.default.ini'))

    # first we apply the default configuration for integrations
    default_config.load_file(os.path.join(ace.SAQ_HOME, 'etc', 'saq.integrations.default.ini'))

    # then if a local configuration exists for this integration, also load that
    default_config.load_file(os.path.join(ace.SAQ_HOME, 'etc', 'saq.integrations.ini'))
    ...
    if 'integrations' in default_config:
        for integration in default_config['integrations'].keys():
            if default_config['integrations'].getboolean(integration):
                ...
                target_config_path = os.path.join(ace.SAQ_HOME, 'etc', f'saq.{integration}.default.ini')
...
def export_encrypted_passwords():
    ...
    with get_db_connection(name=ace.CONFIG['global']['encrypted_passwords_db']) as db:
        c = db.cursor()
        c.execute("""
SELECT
    `key`, `encrypted_value`
...
""", (key,))
        ...
        if ace.ENCRYPTION_PASSWORD is not None:
            from ace.crypto import decrypt_chunk
