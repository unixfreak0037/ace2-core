...
class Event(Base):
    ...
    malware = relationship("ace.database.schema.MalwareMapping", passive_deletes=True, passive_updates=True)
    alert_mappings = relationship("ace.database.schema.EventMapping", passive_deletes=True, passive_updates=True)
    ...
    @property
    def alert_with_email_and_screenshot(self) -> 'ace.database.schema.Alert':
...
class EventMapping(Base):
    ...
    alert = relationship('ace.database.schema.Alert', backref='event_mapping')
    event = relationship('ace.database.schema.Event', backref='event_mapping')
...
class Malware(Base):
    ...
    threats = relationship("ace.database.schema.MalwareThreatMapping", passive_deletes=True, passive_updates=True)
...
class MalwareMapping(Base):
    ...
    malware = relationship("ace.database.schema.Malware")
...
class Alert(Base):
    ...
    disposition_user = relationship('ace.database.schema.User', foreign_keys=[disposition_user_id])
    owner = relationship('ace.database.schema.User', foreign_keys=[owner_id])
    remover = relationship('ace.database.schema.User', foreign_keys=[removal_user_id])
    #observable_mapping = relationship('ace.database.schema.ObservableMapping')
    tag_mappings = relationship('ace.database.schema.TagMapping', passive_deletes=True, passive_updates=True)
    #delayed_analysis = relationship('ace.database.schema.DelayedAnalysis')
...
def sync_observable(observable):
    """Syncs the given observable to the database by inserting a row in the observables table if it does not currently exist.
       Returns the existing or newly created ace.database.schema.Observable entry for the corresponding row."""
    existing_observable = ace.db.query(ace.database.schema.Observable).filter(ace.database.schema.Observable.type == observable.type, 
                                                                       ace.database.schema.Observable.md5 == func.UNHEX(observable.md5_hex)).first()
...
class UserAlertMetrics(Base):
    ...
    alert = relationship('ace.database.schema.Alert', backref='user_alert_metrics')
...
class Observable(Base):
    ...
    tags = relationship('ace.database.schema.ObservableTagMapping', passive_deletes=True, passive_updates=True)
...
class ObservableMapping(Base):
    ...
    alert = relationship('ace.database.schema.Alert', backref='observable_mappings')
    observable = relationship('ace.database.schema.Observable', backref='observable_mappings')
...
class ObservableTagMapping(Base):
    ...
    observable = relationship('ace.database.schema.Observable', backref='observable_tag_mapping')
    tag = relationship('ace.database.schema.Tag', backref='observable_tag_mapping')
...
def add_observable_tag_mapping(o_type, o_value, o_md5, tag):
    ...
    try:
        tag = ace.db.query(ace.database.schema.Tag).filter(ace.database.schema.Tag.name == tag).one()
    except NoResultFound as e:
        ace.db.execute(ace.database.schema.Tag.__table__.insert().values(name=tag))
        ace.db.commit()
        tag = ace.db.query(ace.database.schema.Tag).filter(ace.database.schema.Tag.name == tag).one()
    ...
    if o_md5 is not None:
        try:
            observable = ace.db.query(ace.database.schema.Observable).filter(ace.database.schema.Observable.type==o_type, 
                                                                      ace.database.schema.Observable.md5==func.UNHEX(o_md5)).one()
        except NoResultFound as e:
            if o_value is None:
...
def remove_observable_tag_mapping(o_type, o_value, o_md5, tag):
    ...
    tag = ace.db.query(ace.database.schema.Tag).filter(ace.database.schema.Tag.name == tag).first()
    if tag is None:
        return False
    ...
    if o_md5 is not None:
        observable = ace.db.query(ace.database.schema.Observable).filter(ace.database.schema.Observable.type == o_type,
                                                                  ace.database.schema.Observable.md5 == func.UNHEX(o_md5)).first()
    ...
    if observable is None:
        ...
        observable = ace.db.query(ace.database.schema.Observable).filter(ace.database.schema.Observable.type == o.type,
                                                                  ace.database.schema.Observable.md5 == func.UNHEX(o.md5_hex)).first()
...
class ObservableTagIndex(Base):
    ...
    observable = relationship('ace.database.schema.Observable', backref='observable_tag_index')
    tag = relationship('ace.database.schema.Tag', backref='observable_tag_index')
    alert = relationship('ace.database.schema.Alert', backref='observable_tag_index')
...
class Tag(Base):
    ...
    #def __init__(self, *args, **kwargs):
        #super(ace.database.schema.Tag, self).__init__(*args, **kwargs)
...
class TagMapping(Base):
    ...
    alert = relationship('ace.database.schema.Alert', backref='tag_mapping')
    tag = relationship('ace.database.schema.Tag', backref='tag_mapping')
...
class Remediation(Base):
    ...
    user = relationship('ace.database.schema.User', backref='remediations')
